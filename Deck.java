...
public class Deck {
	...
	{
		Random r= new Random();
		for(int i=0;i<numCardsInDeck;i++)  //Use a nested for loop to loop the cards in the deck
		{                                     // the cards will switch 100 times for every index
			for(int j=0;j<100;j++)
			{
				int w=r.nextInt(numCardsInDeck);
	...
	public void shuffle(int n) 
	{
		...
		for (int i=0; i<cards.length; i++)
		{
			for (int z=0; z<n; z++)
			{
				int a=r.nextInt(numCardsInDeck);
				int b=r.nextInt(numCardsInDeck);
				swap(a,b);
			}
